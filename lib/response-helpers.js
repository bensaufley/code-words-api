...
const config = require('../config'),
      { SocketNotifier, GAME_UPDATED, GAME_REMOVED } = require('./sockets/socket-notifier'),
  ...
  return (game) => {
    ...
    return Promise.all([
      userPlayer ?
        ...
        Promise.resolve(res.status(200).json({ event: GAME_REMOVED, payload: { gameId: game.id } })),
...
const strongParams = (obj, ...keys) => {
  ...
  keys.forEach((key) => {
    if (typeof obj[key] !== 'undefined') response[key] = obj[key];
  });
  return response;
};
