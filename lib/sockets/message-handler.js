...
const config = require('../../config'),
      ...
      { GAME_UPDATED, SocketNotifier } = require('./socket-notifier');
    ...
    return Game.findOne({ where: { id: data.gameId }, include: [Game.Players] })
      .then((g) => {
        ...
        game.players.forEach((player) => {
          new SocketNotifier(player.userId).event(GAME_UPDATED, { game: game.serializeFor(player) });
        });
      });
  }
