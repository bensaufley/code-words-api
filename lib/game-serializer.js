...
module.exports = class GameSerializer {
  ...
  serializeGames(gameId = null) {
    ...
    return Player.findAll({
      ...
      .then((players) => Promise.all(players.map((player) => this.serializeGameForPlayer(player))));
  }

  _findGameWithAssociations(player) {
    return new Promise((resolve, reject) => {
      let hasPlayers = player.game && player.game.players && player.game.players.length > 0,
          hasUsers = hasPlayers && player.game.players.map((p) => p.users).filter(Boolean).length === player.game.players.length;
